...
} from '../../../../src/features/navbar/redux/constants';

import {
  getCategories,
  dismissGetCategoriesError,
  reducer,
} from '../../../../src/features/navbar/redux/getCategories';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('navbar/redux/getCategories', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('dispatches success action when getCategories succeeds', () => {
    ...
    return store.dispatch(getCategories())
      .then(() => {
        ...
        expect(actions[0]).toHaveProperty('type', NAVBAR_GET_CATEGORIES_BEGIN);
        expect(actions[1]).toHaveProperty('type', NAVBAR_GET_CATEGORIES_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when getCategories fails', () => {
    ...
    return store.dispatch(getCategories({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', NAVBAR_GET_CATEGORIES_BEGIN);
        expect(actions[1]).toHaveProperty('type', NAVBAR_GET_CATEGORIES_FAILURE);
  ...
  it('handles action type NAVBAR_GET_CATEGORIES_BEGIN correctly', () => {
  ...
  it('handles action type NAVBAR_GET_CATEGORIES_SUCCESS correctly', () => {
    const prevState = { getCategoriesPending: true };
    const state = reducer(
      prevState,
      { type: NAVBAR_GET_CATEGORIES_SUCCESS, data: {} }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.getCategoriesPending).toBe(false);
  });

  it('handles action type NAVBAR_GET_CATEGORIES_FAILURE correctly', () => {
    const prevState = { getCategoriesPending: true };
    const state = reducer(
      prevState,
      { type: NAVBAR_GET_CATEGORIES_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.getCategoriesPending).toBe(false);
  ...
  it('handles action type NAVBAR_GET_CATEGORIES_DISMISS_ERROR correctly', () => {
